...
// API and platform details
const char* server = "gw-prev.mar2protect.eu";     // Define the server address for API communication
const char* path = "/data/api/devices/586f95";     // Define the path for the API endpoint
const char* client_api_key = "yLryYNOiPN7Z6jNcQ5bVORDbckB8VE0Z"; // Define API key for authentication
...
/*// Define fixed speeds for each pump
float pump1_speed = 128.0;        // Speed for Pump1 (range: 0 to 255)
float pump2_speed = 192.0;        // Speed for Pump2 (range: 0 to 255)
float pump3_speed = 74.0;         // Speed for Pump3 (range: 0 to 255) */
...
HttpClient http(wifiSecure, server, 443);   // Instantiate the HTTP client with secure SSL communication
...
void setup() {
  ...
  Serial.println("SCD30 initialized!");  // Confirm successful initialization
  ...
  Serial.println("VEML7700 initialized!");  // Confirm successful initialization
...
void loop() {
  ...
  // Create JSON payload for API request to send sensor data
  String payload = "{";
  ...
  if (WiFi.status() == WL_CONNECTED) {  // Check if WiFi is connected
    ...
    http.post(path + String("?api_key=") + client_api_key);  // Send a POST request with API key
